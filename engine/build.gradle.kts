    ...
    js(IR) {
        ...
        compilations.named("main") {
            packageJson {
                ...
                // The generated flat facade, not the UMD bundle: Kotlin/JS wraps everything in a
                // `io.github.glandais.*` namespace no `import { … }` can reach, and its own .d.ts
                // references the DTO interfaces without ever declaring them (`external interface`
                // emits no body). `index.*` is generated by `:codegen:generateTsFacade`.
                customField("main", "index.cjs")
                customField("module", "index.mjs")
                customField("types", "index.d.ts")
                customField(
                    "exports",
                    mapOf(
                        "." to
...
val copyTsFacadeToJsPackage =
    tasks.register<Copy>("copyTsFacadeToJsPackage") {
        // Ordering only, not a dependency: the committed directory is the source of truth, and
        // `TsFacadeTest` is what proves it is current. This keeps a single invocation that asks for
        // both from copying the previous contents.
        mustRunAfter(":codegen:generateTsFacade")
...
tasks.register<Exec>("npmPublishJs") {
    ...
    dependsOn("jsBrowserProductionLibraryDistribution", copyReadmeToJsPackage, copyTsFacadeToJsPackage)
    workingDir =
...
//
// `WasiParityTableTest` and `DoorKeyParityTest` read `jsMain`, `wasmWasiMain` and the reference
// host **as text** — the only way to see two source sets at once (see their KDocs). Gradle has no idea, so without this the task stays UP-TO-DATE on
// exactly the edits it exists to watch: a `@JsExport` with no parity row, a `@WasmExport` with no
// host helper, a DTO key added to two doors out of three. `DocumentedFieldCountTest` documented
...
// `WasiOptions.kt` and the demo mirror, and `DoorKeyParityTest` duly failed to notice a key deleted
// from the WASI reader — the guard was real, the task simply never re-ran. It happened again while
// the TypeScript facade was generated: `TsFacadeTest` reported green against a committed file
// broken by hand, because `:codegen`'s inputs did not name it. Adding an extractor means adding its
// source here, in the same commit.
