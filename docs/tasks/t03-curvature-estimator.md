...
feasibility study's recommendation (`racing-line-feasibility.md` §3.1, §6.1): the estimator
...
It fixes three demonstrable bugs (all verified in `racing-line-feasibility.md` §1.2):
...
- The design's §5.1 claim that recorded rings produce `R = 5 m` spikes and 20 km/h caps is
  **unevidenced in the fixtures the repo ships** — the `MIN_RADIUS_M` clamp fires on exactly one
  point per fixture, an endpoint artefact (`racing-line-feasibility.md` §3). Do not cite it as a
