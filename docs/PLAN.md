...
- **Bug #2 — `PointPerDistance` non porté** ✅ corrigé en tâche 30 : le `Enhancer.ts` TS appelle `PointPerDistance.compute(path, ±step, fields)` avant et après `fixElevation` pour densifier la trace à pas constant. Sans cette étape, les paths longs avec des points GPS très espacés sont sous-densifiés, ce qui dégrade la précision de `fixElevation` (moins de mesures DEM) et de `MaxSpeedComputer` (radii calculés sur des fenêtres moins représentatives). Correction : `PointPerDistance` porté en `engine/src/commonMain/kotlin/io/github/glandais/engine/path/PointPerDistance.kt` (object stateless, plan/materialize 2-pass, copyFields/interpolateFields privés, NaN-strict comme `PointPerSecond`). 14 tests `PointPerDistanceTest` couvrant : path vide/1-point, IllegalArgumentException sur `maxDistanceM ≤ 0`, copy verbatim, densification (gap > max → numSegments segments égaux), truncation (`minDist` filtre tout), filtrage sélectif, interpolation lat/lon/elevation/pInputPower, propagation NaN, robustesse sur 200-point random path. Reste à brancher dans `Enhancer` (tâche 31).
