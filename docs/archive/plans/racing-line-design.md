...
> **Archive.** Frozen at its date; it does not describe the code as it is today. For current
> state read [`docs/guides/`](../../guides/) and [`docs/ledgers/`](../../ledgers/) — see
...
> [**R23**](../../ledgers/improvements-ledger.md)), and the corridor, corner detector, offset QP and
...
>
> Corrections to the text below, from the feasibility study (folded into the appendix at the end):
>
...
## Appendix — what the feasibility review got right and wrong

A separate feasibility study reviewed this design against the code on `feat/racing-line` before
any of it was built. It has been folded in here and deleted as a standalone file; this is its
scorecard, settled by what shipped. Archived task specs that cite it by section number — `t03`
most of all — refer to that deleted file.

**The one derivation worth keeping in full.** `R_line = R_c + h·cot²(δ/4)` is asserted, not
derived, in §1 above. Corner vertex `V` at the origin, interior bisector along `+x`, straights at
`±(π−δ)/2`; centreline arc centre at `d₀ = R_c/cos(δ/2)`. A trajectory circle centred at `(t, 0)`
tangent to both outer edges has `ρ = t·cos(δ/2) + h`; tangency to the inner edge at the apex gives
`t − ρ = d₀ − (R_c − h)`. With `c = cos(δ/2)`:

```
ρ = R_c + h·(1+c)/(1−c) = R_c + h·cot²(δ/4)
```

`δ = 180°` → `R_c + h` = 17.5 m; `δ = 90°` → `30 + 2.5·5.828` = 44.57 m — both match §4 and §10-T1,
and the hairpin answer coincides with the independent ceiling of a circle between parallel edges
`2(R_c+h)` apart.

**Right, and decisive:**

- **"Split t03 out and ship it alone."** The curvature estimator was the value. It fixed three real
  defects (the `normalizeAngleDiff` ±π wrap, the point-count window, the `computeBearing` shear),
  touches 40–60 % of ride time rather than the corners only, and shipped as ledger **R23** with no
  QP, no corridor and no coordinate rewrite. The racing line proper (**R24**) then measured
  −0.5 to −0.8 % — an order of magnitude below what §10 asserted, exactly as predicted.
- **"Re-derive §10's aggregate-metric bands."** They fail on every shipped fixture. Per-corner
  radius assertions were and remain the right way to test the stage.
- **"Add WASI to t07."** There are four surfaces, not three. The design listed core, CLI and JS;
  `wasmWasiMain` was already behind and the omission would have shipped a fifth drift — see
  [`../../ledgers/surface-coverage.md`](../../ledgers/surface-coverage.md).
- **"t13 is already shipped"** as ledger R11, and **t09 is the most dangerous thing here.** Neither
  was built.

**Wrong:**

- **"Answer §12 question 1 first — t14 is arguably the true blocking dependency for t08–t10."**
  It is not. `t14` shipped, and inferring road width from the OSM `highway` class is worth
  **0.005 %** against a flat default (ledger **R26**). The corridor remains a global assumption
  and nothing was blocked on it.

**Missed entirely** — the three corrections only implementation could find, all now in this
document's status header: the iteration cap of 12 (it needs ~200), the analytic out–in–out seed
(it makes the solver *slower*), and writing the analytic offset curvature straight out (16–27 %
slower rides until it was re-measured on the materialised path).
