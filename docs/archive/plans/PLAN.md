...
> **Archive.** Ce document a été gelé à sa date et ne décrit pas le code d'aujourd'hui. Pour l'état
> courant, voir [`docs/guides/`](../../guides/) et [`docs/ledgers/`](../../ledgers/). En
> particulier : **la cible Kotlin/Wasm (`wasmJs`) que ce plan suppose n'existe plus** — elle a été
> retirée du projet (Kotlin/Wasm n'est pas WASI, et il lui faut de toute façon un runtime JS dans
> le navigateur, ce que la cible Kotlin/JS couvre déjà). Les cibles réelles sont **JVM, JS (Node),
> JS (browser) et WASI**. Voir [`docs/archive/README.md`](../README.md).
...
et obtenir un GPX corrigé du DEM. Le navigateur reste fonctionnel à l'identique (la branche `decodeBrowser` est inchangée). Le bundle browser ne pulle pas `@jsquash/webp` (webpack externals + `eval('require')`). Pour valider manuellement sous Bun (hors CI) : compiler le bundle Node puis `INTEGRATION=1 bun run engine/build/.../productionExecutable/.../vcyclist-engine.mjs`.
...
[`docs/surface-coverage.md`](../../ledgers/surface-coverage.md).
...
2. `./gradlew :elevation:allTests :engine:allTests` — tests verts en JVM, JS Node, JS browser (invariant trois cibles).
...
À ce stade, l'engine Kotlin est fonctionnellement équivalent à l'engine TS, compilable en JVM/JS/Wasm, et prêt à servir la Phase 9 (demo Compose, elle-même remplacée depuis par la démo Vue/Vite — voir Phase 9).
