 ...
 * nothing connected the sentence to the code. Adding a field is a documented, four-step operation
 * (see `CLAUDE.md`) and this is the step no reviewer can be relied upon to perform.
 *
...
class DocumentedFieldCountTest {
    ...
    private val claims =
        listOf(
            "CLAUDE.md" to Regex("""stores \*\*(\d+) fields × `DoubleArray`\*\* flat"""),
            "README.md" to Regex("""Path model \((\d+) fields × `DoubleArray`\)"""),
